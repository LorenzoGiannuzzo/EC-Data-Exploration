...
def get_single_month_profile(profile_df, month_number):
    ...
    Returns a DataFrame with Q1..Q96 columns indexed by POD."""
...
def run_clustering_for_pods(profile_norm, pod_list, n_clusters=None,
    ...
    Returns: (X_df_with_cluster, optimal_k, details, centroids_df, error_msg)
    """
...
def compute_cluster_stats(X_df):
    ...
    for cl in clusters:
        ...
        # Intra-cluster distance (average pairwise)
        if len(cl_data) > 1:
            intra_dist = pdist(cl_data.values).mean()
        else:
            intra_dist = 0.0

        # Profile variability: mean of std across time steps
...
def plot_cluster_profiles(X_df, k, title_suffix=""):
    ...
    for cl in sorted(X_df["Cluster"].unique()):
        ...
        fig.add_trace(go.Scatter(
            ...
            name=f"Cl.{cl} (n={n})", line=dict(width=2.5),
        ))
        fig.add_trace(go.Scatter(
            x=x_labels + x_labels[::-1],
            y=list((mean_p + std_p).clip(0, 1)) +
              list((mean_p - std_p).clip(0, 1))[::-1],
            fill="toself", opacity=0.10, line=dict(width=0),
...
def plot_cluster_composition(X_df, df_base, ateco_col, level_label):
    ...
    Hovering over any segment shows the FULL composition of that cluster."""
    ...
    comp["Text"] = comp.apply(
        lambda r: f"{int(r['Count'])} ({r['Pct']:.0f}%)" if r["Pct"] >= 5 else "",
        axis=1
    )
    comp["ClusterLabel"] = comp["Cluster"].apply(lambda c: f"Cluster {c}")

    # ---- Build per-cluster full-breakdown hover string ----
    cluster_hover_map = {}
    for cl in sorted(comp["Cluster"].unique()):
        cl_rows = comp[comp["Cluster"] == cl].sort_values("Pct", ascending=False)
        tot = int(cl_rows["Total"].iloc[0])
        parts = [f"<b>Cluster {cl}</b>  ({tot} PODs)<br>"]
        for _, r in cl_rows.iterrows():
            pct = r["Pct"]
            n_blocks = max(1, int(pct / 5))
            parts.append(
                f"{'█' * n_blocks} {r['AtecoLabel']}: "
                f"{int(r['Count'])} ({pct:.1f}%)<br>"
            )
        cluster_hover_map[cl] = "".join(parts)

    # Assign full-cluster hover to every row (so hovering any segment
    # of that cluster shows the complete breakdown)
    comp["FullHover"] = comp["Cluster"].map(cluster_hover_map)

    n_clusters = comp["Cluster"].nunique()

    # Use go.Bar for precise hover control
    ateco_cats = comp["AtecoLabel"].unique().tolist()
    fig = go.Figure()
    for idx, ateco in enumerate(ateco_cats):
        sub = comp[comp["AtecoLabel"] == ateco].sort_values("Cluster")
    ...
    fig.update_layout(
        ...
        height=max(250, n_clusters * 70 + 120),
        margin=dict(t=40, b=20, l=20, r=20),
        legend=dict(font=dict(size=9)),
        hoverlabel=dict(
...
def render_clustering_block(profile_norm, pod_list, label, key_prefix,
                            manual_k=None):
    """Render a complete clustering analysis block for a given POD set.
    Returns True if clustering was performed, False otherwise.
    """
    n_pods = len(pod_list)
    ...
    st.markdown(
        f"**{label}** — {n_with_profile} PODs, k={optimal_k} ({k_source})"
    )
...
def main():
    ...
    # Custom CSS: allow horizontal scroll in checkbox containers, no text wrap
    st.markdown("""
    <style>
    /* Prevent checkbox label text from wrapping and enable horizontal scroll */
    [data-testid="stVerticalBlockBorderWrapper"] [data-testid="stCheckbox"] label {
        white-space: nowrap;
    }
    /* Enable horizontal scrolling in fixed-height containers */
    ...
    # =========================================================================
    # SIDEBAR FILTERS (same as main dashboard)
    # =========================================================================
    with st.sidebar:
        ...
        # Convert to month number: 0 = overall, 1-12 = specific month
        profile_month = MONTH_LABELS.index(profile_period)  # 0-based index = month num
        ...
        if has_potcontr:
            pot_vals = df_unique["POTCONTR_kW"].dropna()
            power_levels = (
    ...
    # =========================================================================
    # PROFILE COMPUTATION (cached, invalidated on tipologia change)
    # =========================================================================
    ...
    # =========================================================================
    # APPLY GLOBAL FILTERS TO df_unique
    # =========================================================================
    ...
    # =========================================================================
    # HIERARCHICAL ATECO SELECTION + CLUSTERING (wrapped in fragment)
    # =========================================================================
...
# Wrapped in @st.fragment so that checkbox clicks only re-render this section,
# NOT the entire page (sidebar, header, data loading stay solid).
# ==============================================================================
...
def ateco_clustering_section(df_base, profile_norm, df_unique, manual_k,
    ...
    # ===================== LEVEL 2 (collapsible, parent checkbox) =====================
    selected_l2 = []
    with col_l2:
        ...
        if not selected_l1:
            ...
            if l2_data.empty:
                ...
                with sa2:
                    if st.button("Select All", key="sa_l2",
                                 use_container_width=True):
                        ...
                        st.rerun(scope="fragment")
                with da2:
                    if st.button("Deselect All", key="da_l2",
                                 use_container_width=True):
                        for c in all_l2_codes:
                ...
                with st.container(height=350):
                    for l1_code in selected_l1:
                        ...
                        # Check if all children are selected → sync parent
                        all_kids_on = all(
                            st.session_state.get(f"cb_l2_{c}", False)
                            for c in child_codes_l2
                        )
                        prev_par = st.session_state.get(
                            f"_pv_l2par_{l1_code}", None
                        )
                        ...
                        # Detect parent toggle → cascade to children
    ...
    # ===================== LEVEL 3 (collapsible, parent checkbox) =====================
    selected_l3 = []
    with col_l3:
        ...
        if not selected_l2:
            ...
            l3_data = (
                df_l2_filtered.groupby(["ATECO_L1", "ATECO_L2", "ATECO_L3"])[
                    "POD"
                ].nunique()
                .reset_index().rename(columns={"POD": "N"})
            )
            ...
            if l3_data.empty:
                ...
                with sa3:
                    if st.button("Select All", key="sa_l3",
                                 use_container_width=True):
                        ...
                        st.rerun(scope="fragment")
                with da3:
                    if st.button("Deselect All", key="da_l3",
                                 use_container_width=True):
                        for c in all_l3_codes:
                ...
                with st.container(height=350):
                    ...
                    for l2_code in selected_l2:
                        ...
                        # Check if all children selected → sync parent
                        all_kids_on_l3 = all(
                            st.session_state.get(f"cb_l3_{c}", False)
                            for c in child_codes_l3
                        )
                        prev_par_l3 = st.session_state.get(
                            f"_pv_l3par_{l2_code}", None
                        )
                        ...
                        # Detect parent toggle → cascade
                        if (prev_par_l3 is not None
                                and parent_val_l3 != prev_par_l3):
                            for c in child_codes_l3:
                                st.session_state[f"cb_l3_{c}"] = parent_val_l3
                            st.session_state[f"_pv_l3par_{l2_code}"] = \
                                parent_val_l3
                            st.rerun(scope="fragment")
                        st.session_state[f"_pv_l3par_{l2_code}"] = \
                            parent_val_l3
    ...
    for lname, codes, pods, _ in active_levels:
        ...
        summary_parts.append(
            f"**{lname}**: {codes_str} ({n_prof} PODs with profile)"
        )
    ...
    has_cached = (
        cached_fp == sel_fingerprint
        and "_cl_results" in st.session_state
    )

    btn_col1, btn_col2, btn_col3 = st.columns([1, 1, 2])
    with btn_col1:
        run_clicked = st.button(
            "▶ Run Clustering", type="primary", key="run_cl_btn"
        )
    with btn_col2:
        if has_cached:
    ...
    if run_clicked:
        ...
        for level_name, codes, pods, ateco_col in active_levels:
            ...
            with st.spinner(
                f"Clustering {level_name} ({len(pods_with_profile)} PODs)..."
            ):
    ...
    for col_widget, (level_name, res) in zip(cols, level_results):
        with col_widget:
            ...
            # ---- Cluster composition chart ----
            if len(codes) >= 2:
                with st.expander("📊 Cluster Composition by ATECO",
                                 expanded=True):
            ...
            for mc, (mname, mval) in zip(m_cols, global_metrics.items()):
                mc.metric(mname, mval,
                          help=METRIC_HELP.get(mname, ""))
    ...
    if st.button("Prepare Export ZIP", type="primary", key="frag_export_btn"):
        ...
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
            for level_name, res in valid_results.items():
                ...
                with pd.ExcelWriter(excel_buf, engine="openpyxl") as writer:
                    ...
                    pod_cl.to_excel(writer, index=False,
                                    sheet_name="POD_List")
                ...
                zf.writestr(f"{level_name}_k{k}_stats.xlsx",
                            excel_buf.getvalue())

                # PNG
                try:
                    fig = plot_cluster_profiles(X_df, k, f" | {level_name}")
                    png_bytes = fig_to_png_bytes(fig, width=1600, height=800)
                    zf.writestr(f"{level_name}_k{k}_profiles.png", png_bytes)
                except Exception as e:
                ...
                if k >= 2:
                    try:
                        ...
                        png_h = fig_to_png_bytes(fig_h, width=800, height=600)
                        zf.writestr(f"{level_name}_k{k}_pearson.png", png_h)
                    except Exception as e:
                ...
                if len(res.get("codes", [])) >= 2:
                    try:
                        ...
                        png_c = fig_to_png_bytes(fig_c, width=1200, height=600)
        ...
        if png_errors:
            st.warning(f"{len(png_errors)} PNG export issues. Excel files OK.")
